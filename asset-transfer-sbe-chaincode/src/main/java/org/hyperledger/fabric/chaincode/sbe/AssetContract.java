package org.hyperledger.fabric.chaincode.sbe;
...
public final class AssetContract implements ContractInterface {
    ...
    public Asset createAsset(final Context ctx, final String assetId, final int value, final String owner) {
        ...
        if(AssetExists(ctx, assetId)) {
            String errorMessage = String.format("Asset %s alreay exists", assetId);
     ...
     */
    private boolean AssetExists(Context ctx, String assetId) {
        ChaincodeStub stub = ctx.getStub();
        String assetJSON = stub.getStringState(assetId);
     ...
     */
    private static String getClientOrgId(Context ctx) {
     ...
     */
    private static void setStateBasedEndorsement(Context ctx, String assetId, String[] ownerOrgs) {
    ...
    private static Policies.SignaturePolicy nOutOf(int n, List<SignaturePolicy> policies) {
