...
public class AssetTransfer implements ContractInterface{
    ...
    /**
     * AssetCollection에서 지정된 ID로 공개된 자산 세부 정보를 검색한다.
     *
    ...
    /**
     * collection에서 지정된 ID로 asset의 AssetPrivateDetails 세부 정보를 검색한다. 
     *
     * @param ctx        the transaction context
    ...
    /**
     * ReadTransferAgreement는 collection의 transferAgreement 에서 buyer's identity를 가져온다.
     *
     * @param ctx     the transaction context
     * @param assetID the ID of the asset
     * @return the AssetPrivateDetails from the collection, if there was one
    ...
    public TransferAgreement readTransferAgreement(final Context ctx, final String assetID) {
        ...
        return new TransferAgreement(assetID, new String(buyerIdentity, StandardCharsets.UTF_8));
    }
        ...
        try(QueryResultsIterator<KeyValue> results = stub.getPrivateDataByRange(ASSET_COLLECTION_NAME, startKey, endKey)) {
            for (KeyValue result : results) {
                if (result.getStringValue() == null || result.getStringValue().length() == 0) {
                    System.out.printf("Invalid Asset json: %s\n", result.getStringValue());
                    continue;
                }
                Asset asset = Asset.deserialize(result.getStringValue());
                queryResults.add(asset);
                System.out.printf("QueryResults: " + asset.toString());
            }
        }
    ...
    public Asset createAsset(final Context ctx) {
        ...
        int size = 0;
        try {
            JSONObject json = new JSONObject(new String(transientAssetJSON, StandardCharsets.UTF_8));
    ...
    public void agreeToTransfer(final Context ctx) {
        ...
        if (assetPriv.getAppraisedValue() <= 0) {
        ...
        stub.putPrivateData(ASSET_COLLECTION_NAME, aggKey.getObjectType(), clientID);
    }
    ...
    public void transferAsset(final Context ctx) {
        ...
        System.out.printf("TransferAsset: verify asset %s exist\n", assetID);
        byte[] assetJSON = stub.getPrivateData(ASSET_COLLECTION_NAME, assetID);
        ...
        //delete the key form owners collection
